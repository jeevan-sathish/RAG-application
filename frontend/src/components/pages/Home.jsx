    ...
    <div className='w-full h-[90vh] bg-gray-900 flex flex-row gap-2  '>
       ...
       <div className='contentPage w-[40%] h-[85vh] shadow-2xl shadow-blue-900 rounded-2xl m-[20px] border border-gray-800 bg-black p-[30px]'>
          <div className='w-full h-[70px] flex flex-row justify-center items-center'>
             <p className='text-[22px] text-amber-300'> <span className='text-[35px] font-extrabold'>RAG</span>(Retrieval Augmented Generation)</p>
          </div>
          <p className='text-gray-600 text-[14px] hover:animate-pulse'   style={{ fontFamily: "Fira Code, monospace" }}>{ragExplanation}</p>
       </div>

       <div className='w-[55%] h-[85vh] bg-tranparent m-[20px] flex flex-col gap-[30px] justify-center items-center p-[40px]'>
          <p className='text-[50px] text-center text-white leading-13.5'>Wellcome,Try The new <span className='text-blue-500'>RAG</span> Tech</p>
            <div className='w-[80%] min-h-[200px] p-[20px] rounded-2xl bg-gray-800 border border-black flex flex-col'>
                ...
                <div className='w-full flex flex-row gap-4 mt-[10px]'>
                  <input
                   ...
                   value={text}
                   onChange={handleText} 
                   className='w-[300px] h-[30px] pl-[30px] text-green-400  rounded-[5px] border border-black' placeholder='ask about Rag?'/>
                  <button onClick={handleSubmit} className='text-red-600 font-extrabold animate-pulse hover:scale-120'>fetch()</button>
                  <button onClick={handleClear} className='text-teal-300'>clear()</button>
                </div>
            ...
            <div className='w-[85%] h-[50px] flex flex-row justify-center  gap-[15px] items-center shrink-0 p-[10px] bg-black rounded-[5px] text-white'>
                  ...
                  <h1>chunk_size : <span className='text-red-500'>{chunkSize}</span></h1>
                    <h1>vector_Len : <span className='text-red-500'>{vectorLen}</span></h1>
                      <h1>vector_len[0]: <span className='text-red-500'>{eachVectorlen}</span></h1>
                </div>
